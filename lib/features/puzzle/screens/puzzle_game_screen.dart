...
if (widget.isCustomImage) {
  final savedSize = savedGameData?["customGridSize"];
...
static const String customPuzzleId = "custom_island";
...
return savedSeconds + stopwatch.elapsed.inSeconds;
}
...
final levels = PuzzleLevelData.getLevels(widget.island!.id);
...
await PuzzleProgressManager.clearPrivateIslandGameState();
}
...
final savedPuzzleId = saved["puzzleId"];
final savedLevelId = saved["levelId"];
final savedImagePath = saved["customImagePath"];
...
final isSameGame =
    savedPuzzleId == customPuzzleId &&
    savedLevelId == currentLevelId &&
    savedImagePath == widget.customImagePath;
...
// ----------------------------------------------------------
// 🛡️ التأكد أن ملف الصورة موجود فعليًا قبل عرض خيار
// الاستكمال. إذا لم تعد الصورة موجودة (تم حذفها من
// الجهاز)، ننظف الحفظ القديم بأمان بدل محاولة فتح
// ملف غير موجود.
// ----------------------------------------------------------

final imagePath = widget.customImagePath;
...
try {
  imageExists = await File(imagePath).exists();
} catch (_) {
  imageExists = false;
}
          ...
          onPressed: () {
            AdsManager().showRewardedAd(
              ...
              onAdFailed: () {
                ...
                // الإعلان غير متوفر، يسمح بالاستمرار مباشرة.
                Navigator.pop(
                  dialogContext,
                  true,
                );
...
final samePuzzle = saved["puzzleId"] == normalPuzzleId;

final sameLevel = saved["levelId"] == currentLevelId;
...
// ============================================================
// ⚠️ الصورة غير متوفرة (الجزيرة الخاصة فقط)
// ============================================================

Future<void> _handleMissingCustomImage() async {
await PuzzleProgressManager.clearInvalidPrivateIslandSave();
...
await showDialog<void>(
  ...
  builder: (dialogContext) {
    return AlertDialog(
      ...
      content: const Text(
        "الصورة الخاصة بهذه اللعبة لم تعد متوفرة على الجهاز.",
      ),
      actions: [
        TextButton(
          onPressed: () {
...
// ----------------------------------------------------------
// 🏝️ التحقق من وجود ملف الصورة الخاصة قبل أي محاولة
// لفتحه، حتى لا يحدث crash إذا حُذف الملف من الجهاز.
// ----------------------------------------------------------
...
if (widget.isCustomImage) {
  ...
  if (!exists) {
    await _handleMissingCustomImage();
    return;
  }
}
...
if (savedGameData != null) {
  ...
  lastPlacedCount = controller.pieces
      .where(
        (p) => p.isPlaced,
      )
      .length;

  moves = savedGameData!["moves"] is num
      ? (savedGameData!["moves"] as num).toInt()
      : 0;

  savedSeconds = savedGameData!["seconds"] is num
      ? (savedGameData!["seconds"] as num).toInt()
      : 0;
...
: normalPuzzleId,
  ...
  "pieces": controller.pieces.map((piece) {
    return {
      "id": piece.id,
      "row": piece.row,
      "column": piece.col,
...
final state = _buildSaveData();

// ----------------------------------------------------------
// 🏝️ نظام حفظ الجزيرة الخاصة فقط
// ----------------------------------------------------------

await PuzzleProgressManager.savePrivateIslandGameState(
  state,
);
...
if (widget.isCustomImage) {
  ...
  await PuzzleProgressManager.clearPrivateIslandImage();
} else {
  // 🧩 البازل العادي
  await PuzzleProgressManager.clearProgress();
}
    ...
    pageBuilder: (_, animation, secondaryAnimation) {
      return VictoryScreen(
        ...
        levelNumber: widget.isCustomImage
            ? 10
            : widget.level!.levelNumber,
        isFinalLevel: isFinalLevelOfIsland,
                  ...
                  AdsManager().showRewardedAd(
                    ...
                    onAdFailed: () {
                      ...
                      ScaffoldMessenger.of(context)
                        ...
                        const SnackBar(
                          content: Text(
                            "إعادة التجميع غير متوفر حاليًا، حاول لاحقًا",
                            textAlign:
                                TextAlign.center,
                          ),
                          behavior:
...
if (puzzleCreated && !gameFinished) {
  if (widget.isCustomImage) {
    ...
    PuzzleProgressManager.savePrivateIslandGameState(
      state,
    );

    if (widget.customImagePath != null) {
      PuzzleProgressManager.savePrivateIslandImagePath(
        widget.customImagePath!,
      );
    }
  } else {
