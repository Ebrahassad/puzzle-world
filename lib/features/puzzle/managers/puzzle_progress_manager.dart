...
class PuzzleProgressManager {
  ...
  //==================================================
  // 🏝️🗑️ حذف حالة لعبة الجزيرة الخاصة فقط
  //==================================================
  ...
  //==================================================
  // 🏝️🔍 التحقق من صلاحية حفظ الجزيرة الخاصة
  //
  // "صالح" يعني:
  //  1. يوجد حفظ حالة بازل (pieces/moves/seconds..).
  //  2. يوجد مسار صورة مرتبط به.
  //  3. الصورة فعلًا موجودة على الجهاز.
  //
  // هذه الدالة لا تنشئ أي مفتاح جديد، فقط تقرأ
  // المفاتيح الموجودة أصلًا (privateIslandGameStateKey
  // و privateIslandImagePathKey) وتتحقق منهما معًا،
  // حتى لا نحاول أبدًا استرجاع بازل لصورة غير موجودة.
  //==================================================

  static Future<bool> hasValidPrivateIslandSave() async {
    final state = await loadPrivateIslandGameState();

    if (state == null || state.isEmpty) {
      return false;
    }

    final imagePath = await getPrivateIslandImagePath();
  ...
  //==================================================
  // 🏝️🗑️ حذف حفظ الجزيرة الخاصة غير الصالح
  //
  // تُستخدم عندما تكون حالة البازل محفوظة، لكن الصورة
  // حُذفت من الجهاز (أو العكس)، لتنظيف الحفظ بأمان
  // بدون أي محاولة لفتح ملف غير موجود.
  //==================================================

  static Future<void> clearInvalidPrivateIslandSave() async {
    await clearPrivateIslandGameState();
    await clearPrivateIslandImage();
  }
  ...
  static Future<void> resetProgress() async {
    ...
    // (privateIslandGameStateKey و privateIslandImagePathKey
    // يبقيان كما هما).
  }
